  ...
  if (isLoadingSession || isAuthed) {
    return (
      <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-slate-950 px-4 py-8">
        ...
        <div className="relative z-10 w-full max-w-md rounded-2xl border border-slate-800/70 bg-slate-950/80 px-5 py-6 text-center shadow-[0_24px_70px_rgba(0,0,0,0.75)] backdrop-blur-2xl">
          <p className="text-sm font-medium text-slate-100">
            {isLoadingSession ? "Checking your session…" : "You’re already signed in."}
          </p>
          <p className="mt-2 text-[11px] text-slate-400">
            {isLoadingSession
              ? "Please wait a moment while we prepare your dashboard."
              : "Redirecting you to your dashboard. If nothing happens, use the button below."}
          </p>
          {!isLoadingSession && (
            <Link
              href="/dashboard"
              className="mt-4 inline-flex items-center justify-center rounded-2xl bg-sky-500 px-4 py-2 text-xs font-medium text-slate-950 shadow-sm transition hover:bg-sky-400"
            >
              Go to dashboard
            </Link>
          )}
      ...
      >
        ...
        <div className="flex flex-1 items-stretch">
          ...
          >
              ...
              <header>
                ...
                <p className="mt-1 text-[11px] text-slate-400">
                  {mode === "signin"
                    ...
                    : "Set up a secure login to unlock dashboards tuned to your spending."}
                </p>
              </header>
              <div className="hidden rounded-full bg-slate-900/80 p-1 text-[11px] text-slate-400 sm:inline-flex">
                <button
                  type="button"
            ...
            <div className="mb-3 inline-flex w-full justify-center rounded-full bg-slate-900/80 p-1 text-[11px] text-slate-400 sm:hidden">
              <button
                type="button"
                onClick={() => setMode("signin")}
                className={
                  "flex-1 rounded-full px-2.5 py-1 transition " +
                  (mode === "signin"
                    ? "bg-slate-50 text-slate-900"
                    : "text-slate-300 hover:text-slate-50")
                }
              ...
              {mode === "signup" && (
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full rounded-2xl border border-slate-700 bg-slate-900/80 px-3 py-2.5 text-sm text-slate-50 outline-none transition placeholder:text-slate-500 focus:border-sky-500 focus:ring-2 focus:ring-sky-500/30"
